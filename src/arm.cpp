...
void Arm::solve(Point3f goal_point, int life_count) {
    ...
    while (curr_err > err_margin) {
        ...
        for(int i=0; i<3*segment_size; i+=3) {
            Matrix<float, 1, 3> row_theta = compute_jacobian_segment(i/3, goal_point, segments[i/3]->get_right());
            Matrix<float, 1, 3> row_phi = compute_jacobian_segment(i/3, goal_point, segments[i/3]->get_up());
            Matrix<float, 1, 3> row_z = compute_jacobian_segment(i/3, goal_point, segments[i/3]->get_z());
